 ...
 *
 */
public class PatternSpecialOutLinkPlugin {
	public static final Logger LOG = LoggerFactory.getLogger("org.apache.nutch.parse.html.PatternSpecialOutLinkPlugin");
	private Outlink[] outlink;
	private Pattern[] urlPattern = null;
	private Pattern[] filterPattern = null;
	private String url=null;
	
	public PatternSpecialOutLinkPlugin(Outlink[] outlinks, WebPage page) {
		this.outlink = outlinks;
		this.url = page.getBaseUrl() != null?page.getBaseUrl().toString():null;
		String param = page.nodeConfig.getConf("parase.plugin.outlink.name.pattern");
		if (param == null || param.trim().length()<=0){
			LOG.warn("param 'parase.plugin.outlink.name.pattern' is null");
			return;
		}
		...
		if (ups.length !=2 ){
			LOG.warn("param 'parase.plugin.outlink.name.pattern' should contain pattern and value");
			return;
		}
	...
	public Outlink[] getOutLink(){
		...
		if (!isPattern){
			LOG.info("Don't need to pattern");
			return this.outlink;
		}
		...
		for (int i = 0; i < this.outlink.length; i++) {
			for (int j = 0; j < this.filterPattern.length; j++) {
				...
				if (null != this.filterPattern[j] && this.filterPattern[j].matcher(this.outlink[i].getToUrl()).matches()){
					LOG.info("pattern_outlinkurl="+this.outlink[i].getToUrl() +", pattern:"+this.filterPattern[j]);
