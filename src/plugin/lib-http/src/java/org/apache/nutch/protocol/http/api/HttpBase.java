...
public abstract class HttpBase implements Protocol {
	...
	protected String userAgent = getAgentString("NutchCVS", null, "Nutch", "http://lucene.apache.org/nutch/bot.html",
	...
	protected String accept = "text/html,application/xml;q=0.9,application/xhtml+xml,text/xml;q=0.9,text/plain;q=0.8,image/png,*/*;q=0.5";
	...
	public void setConf(Configuration conf) {
		...
		this.timeout = conf.getInt("http.timeout", 30000);
		this.maxContent = conf.getInt("http.content.limit", 64 * 1024);
		this.userAgent = getAgentString(conf.get("http.agent.name"), conf.get("http.agent.version"), conf.get("http.agent.description"),
	...
	public ProtocolOutput getProtocolOutput(String url, WebPage page) {
		try {
			...
			if (response == null) {
				throw new Exception("未知错误,空值返回");
			}
			int code = response.getCode();
			byte[] content = response.getContent();
			Content c = new Content(u.toString(), u.toString(), (content == null ? EMPTY_CONTENT : content), response
					.getHeader("Content-Type"), response.getHeaders(), mimeTypes);
			if (code == 200 || code == 201) { // got a good response
				if (content == null)
					page.setFetchInterval(0);
				return new ProtocolOutput(c); // return it
			} else if (code == 410) { // page is gone
				return new ProtocolOutput(c, ProtocolStatusUtils.makeStatus(ProtocolStatusCodes.GONE, "Http: " + code + " url=" + url));
				...
				return new ProtocolOutput(c, ProtocolStatusUtils.makeStatus(ProtocolStatusCodes.ACCESS_DENIED, "Authentication required: "
						+ url));
				...
				return new ProtocolOutput(c, ProtocolStatusUtils.makeStatus(ProtocolStatusCodes.EXCEPTION, "Http code=" + code + ", url="
						+ u));
			}
		} catch (Exception e) {
			System.out.print("Httpbase 210:");
	...
	public byte[] processGzipEncoded(byte[] compressed, URL url) throws IOException {
		...
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("fetched " + compressed.length + " bytes of compressed content (expanded to " + content.length + " bytes) from "
					+ url);
		}
		return content;
	}
	...
	protected static void main(HttpBase http, String[] args) throws Exception {
		...
		ProtocolOutput out = http.getProtocolOutput(url, new WebPageIndex());
