...
/**
 * This class handles the parsing of <code>robots.txt</code> files. It emits RobotRules objects, which describe the download permissions as
 * described in RobotRulesParser.
 * 
 * @author Tom Pierce
...
public class RobotRulesParser implements Configurable {
     ...
     */
		...
		/**
		 * Returns <code>false</code> if the <code>robots.txt</code> file prohibits us from accessing the given <code>url</code>, or
		...
		/**
		 * Returns <code>false</code> if the <code>robots.txt</code> file prohibits us from accessing the given <code>path</code>, or
		 * <code>true</code> otherwise.
		 */
	...
	/**
	 * Creates a new <code>RobotRulesParser</code> which will use the supplied <code>robotNames</code> when choosing which stanza to follow
	 * in <code>robots.txt</code> files. Any name in the array may be matched. The order of the <code>robotNames</code> determines the
	 * precedence- if many names are matched, only the rules associated with the robot name having the smallest index will be used.
	 */
	...
	/**
	 * Returns a {@link RobotRuleSet} object which encapsulates the rules parsed from the supplied <code>robotContent</code>.
	 */
	...
	/**
	 * Returns a <code>RobotRuleSet</code> object appropriate for use when the <code>robots.txt</code> file is empty or missing; all
	...
	/**
	 * Returns a <code>RobotRuleSet</code> object appropriate for use when the <code>robots.txt</code> file is not fetched due to a
	...
	private RobotRuleSet getRobotRulesSet(HttpBase http, URL url) {
		...
		if (robotRules == null) { // cache miss
			...
			try {
				Response response = http.getResponse(new URL(url, "/robots.txt"), new WebPage(), true);
