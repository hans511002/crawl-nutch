...
public class ZipTextExtractor {
	...
	public String[] extractText(InputStream input, String url, List<Outlink> outLinksList) throws IOException {
			...
			if (!entry.isDirectory()) {
				...
				if (i != -1) {
					...
					try {
						...
						page.setBaseUrl(new org.apache.avro.util.Utf8(newurl));
						page.setContent(ByteBuffer.wrap(content.getContent()));
						page.setContentType(new Utf8(contentType));
						Parse parse = new ParseUtil(this.conf).parse(newurl, page); // ().parse(content).get(content.getUrl());
		...
		return new String[] { resultTitle, resultText };
	}
