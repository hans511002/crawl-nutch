...
public class TestZipParser extends TestCase {
	...
	public void testIt() throws ProtocolException, ParseException {
		...
		for (int i = 0; i < sampleFiles.length; i++) {
			...
			content = protocol.getProtocolOutput(urlString, page).getContent();
			new ParseUtil(conf).process(urlString, page);// parseByExtensionId("parse-zip", content).get(content.getUrl());
			assertTrue(page.getText().equals(expectedText));
		}
	}
