...
public class HttpResponseComponentSina implements IHttpLogin {

	public static void main(String[] args) {
		PoolingClientConnectionManager poolingClientConnectionManager = new PoolingClientConnectionManager();
		DefaultHttpClient client = new DefaultHttpClient(poolingClientConnectionManager);
		HttpResponseComponentSina sina = new HttpResponseComponentSina();
		WebPage page = new WebPage();
		page.nodeConfig = new UrlNodeConfig();
		page.nodeConfig.loginPassword = "1QAZ-PL,";
		page.nodeConfig.loginUserName = "939636293@qq.com";
		page.nodeConfig.loginValidateAddress = "http://account.weibo.com/set/index?topnav=1&wvr=5";
		try {
			sina.login(client, page);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public boolean login(HttpClient client, WebPage page) throws Exception {
		TestLogUtil.logPrefixBlank("Sina Login");
		...
		TestLogUtil.logPrefixBlank("Sina Login info; username:" + username + ",password:" + password + ",loginValidatePath:"
				+ loginValidatePath);
		HttpPost post = null;
		HttpGet getMethod = null;
		try {
			/** 获得rsaPubkey,rsakv,servertime等参数值，此获取参数值的方法的形式 **/
			HashMap<String, String> params = preLogin(encodeAccount(username), client);

			/******** 登录操作 *********/
			post = new HttpPost("http://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.5)");
			...
			se.eval(getJs());
			String pass = "";
			TestLogUtil.logPrefixBlank("Sina Login encode password is Invocable :" + (se instanceof Invocable));
			if (se instanceof Invocable) {
				Invocable invoke = (Invocable) se;
				// 调用preprocess方法，并传入两个参数密码和验证码
				pass = invoke.invokeFunction("getpass", password, params.get("servertime"), nonce, params.get("pubkey")).toString();
				TestLogUtil.logPrefixBlank("Sina Login encode password :" + pass);
			}
			...
			byte[] content = HttpClientUtils.getResponseContent(response);
			String entity = new String(HttpClientUtils.extractContent(response, content));
			TestLogUtil.logPrefixBlank("Sina Login real url before:" + entity);
			if (entity.replace("\"", "").indexOf("retcode=0") > -1) {
				...
				TestLogUtil.logPrefixBlank("Sina Login real url:" + url);
				// 获取到实际url进行连接
				TestLogUtil.logPrefixBlank("Sina Login real url connect");
				getMethod = new HttpGet(url);
				response = client.execute(getMethod);
				// entity = EntityUtils.toString(response.getEntity());
				// TestLogUtil.logPrefixBlank("Sina Login real url content:"+entity);
		...
		TestLogUtil.logPrefixBlank("Sina Login result:" + false);
		return false;
	}

	@Override
	public boolean isLogin(HttpClient client, WebPage page, String loginValidateAddress) {
		TestLogUtil.logPrefixBlank("Sina validate Login");
		if (null == loginValidateAddress || loginValidateAddress.trim().length() <= 0) {
			return false;
		}

		HttpGet getMethod = new HttpGet(loginValidateAddress);
		try {
			...
			HttpEntity httpEntity = response.getEntity();
			byte[] content = HttpClientUtils.getResponseContent(response);
			String entitys = new String(HttpClientUtils.extractContent(response, content));
			TestLogUtil.logPrefixBlank("Sina Login validate address content:");
			TestLogUtil.logPrefixBlank(entitys);
			if (HttpStatus.SC_OK == response.getStatusLine().getStatusCode()) {
				TestLogUtil.logPrefixBlank("Sina validate Login result:" + true);
				return true;
			}
			EntityUtils.consume(httpEntity);
		...
		TestLogUtil.logPrefixBlank("Sina validate Login result:" + false);
		return false;
	}
	 ...
	 * @throws IOException
	 */
	private String get(String url, HttpClient client) throws IOException {
		HttpGet get = null;
		String result = "";
		try {
			get = new HttpGet(url);
	 ...
	 * @throws IOException
	 */
	private HashMap<String, String> preLogin(String unameBase64, HttpClient client) throws IOException {
		String url = "http://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su=&rsakt=mod&client=ssologin.js(v1.4.5)&_="
				+ "_=" + new Date().getTime();
		return this.getParaFromResult(get(url, client));
	}
	 ...
	 * @throws IOException
	 */
	private String dump(HttpResponse response, HttpEntity entity) throws IOException {
		// BufferedReader br = new BufferedReader(new InputStreamReader(entity.getContent(),"UTF-8"));
		byte[] content = HttpClientUtils.getResponseContent(response);
		return new String(HttpClientUtils.extractContent(response, content));
