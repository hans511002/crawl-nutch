...
/**
 * Mapping definitions for HBase. Thread safe. It holds a definition for a single table.
 */
public class HBaseMapping {
	...
	/**
	 * A builder for creating the mapper. This will allow building a thread safe {@link HBaseMapping} using simple immutabilty.
	 * 
	 */
	public static class HBaseMappingBuilder {
		...
		public void addFamilyProps(String tableName, String familyName, String compression, String blockCache, String blockSize,
				String bloomFilter, String maxVersions, String minVersions, String timeToLive, String inMemory, String TTL) {
			...
			if (minVersions != null)
				columnDescriptor.setMinVersions(Integer.parseInt(minVersions));
			else
			...
			if (TTL != null)
				columnDescriptor.setTimeToLive(Integer.parseInt(TTL));
		 ...
		 */
		public HBaseMapping build(long maxFileSize) {
			if (tableName == null)
			...
			if (maxFileSize > 8 * 1024 * 1024)
				tableDescriptors.setMaxFileSize(maxFileSize);
