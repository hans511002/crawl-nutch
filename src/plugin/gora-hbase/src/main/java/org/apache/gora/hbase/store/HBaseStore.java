...
public class HBaseStore<K, T extends PersistentBase> extends DataStoreBase<K, T> implements Configurable {
	...
	public void initialize(Class<K> keyClass, Class<T> persistentClass, Properties properties) {
		try {
			...
			try {
				...
				LOG.warn(DEPRECATED_MAPPING_FILE + " is deprecated, please rename the file to " + DEFAULT_MAPPING_FILE);
	...
	public void put(K key, T persistent) {
		try {
			...
			for (int i = 0; iter.hasNext(); i++) {
				...
				if (hcol == null) {
					throw new RuntimeException("HBase mapping for field [" + persistent.getClass().getName() + "#" + field.name()
	...
	public long deleteByQuery(Query<K, T> query) {
		try {
			...
			boolean isAllFields = Arrays.equals(fields, getBeanFactory().getCachedPersistent().getFields());
	...
	public List<PartitionQuery<K, T>> getPartitions(Query<K, T> query) throws IOException {
		...
		for (int i = keys.getFirst().length - 1; i >= 0; i--) {
			...
			if ((startRow.length == 0 || keys.getSecond()[i].length == 0 || Bytes.compareTo(startRow, keys.getSecond()[i]) < 0)
				...
				K startKey = Arrays.equals(HConstants.EMPTY_START_ROW, splitStart) ? null : HBaseByteInterface.fromBytes(keyClass,
						splitStart);
				K endKey = Arrays.equals(HConstants.EMPTY_END_ROW, splitStop) ? null : HBaseByteInterface.fromBytes(keyClass, splitStop);
	...
	public org.apache.gora.query.Result<K, T> execute(Query<K, T> query) {
		try {
			...
			if (query.getStartKey() != null && query.getStartKey().equals(query.getEndKey())) {
				...
				org.apache.gora.query.Result<K, T> result = new HBaseScannerResult<K, T>(this, query, scanner);
	...
	@SuppressWarnings( { "unchecked", "rawtypes" })
	public T newInstance(Result result, String[] fields) throws IOException {
	...
	private void setField(T persistent, Field field, byte[] val) throws IOException {
	...
	private HBaseMapping readMapping(String filename) throws IOException {
		...
		try {
			...
			for (Element tableElement : tableElements) {
				...
				String maxFs = tableElement.getAttributeValue("maxFileSize");
				if (maxFs != null)
					maxFileSize = Long.parseLong(maxFs);
				...
				for (Element fieldElement : fieldElements) {
					...
					String minVersions = fieldElement.getAttributeValue("minVersions");
					String timeToLive = fieldElement.getAttributeValue("timeToLive");
					String inMemory = fieldElement.getAttributeValue("inMemory");
					String TTL = fieldElement.getAttributeValue("TTL");

					mappingBuilder.addFamilyProps(tableName, familyName, compression, blockCache, blockSize, bloomFilter, maxVersions,
							minVersions, timeToLive, inMemory, TTL);
				}
			}
			...
			for (Element classElement : classElements) {
				if (classElement.getAttributeValue("keyClass").equals(keyClass.getCanonicalName())
					...
					if (!tableName.equals(tableNameFromMapping)) {
						LOG.info("Keyclass and nameclass match but mismatching table names " + " mappingfile schema is '"
								+ tableNameFromMapping + "' vs actual schema '" + tableName + "' , assuming they are the same.");
		...
		return mappingBuilder.build(maxFileSize);
	}
