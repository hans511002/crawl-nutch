...
/**
 * Unit tests for ExtParser. First creates a temp file with fixed content, then fetch and parse it using external command 'cat' and 'md5sum'
 * alternately for 10 times. Doing so also does a light stress test for class CommandRunner.java (as used in ExtParser.java).
 * 
...
public class TestExtParser extends TestCase {
	...
	protected void setUp() throws ProtocolException, IOException {
		...
		content = protocol.getProtocolOutput(urlString, new WebPage()).getContent();
		protocol = null;
	}
	...
	public void testIt() throws ParseException {
		...
		for (int i = 0; i < 10; i++) {
			...
			new ParseUtil(conf).process(content.getUrl(), page);
			...
			content.setContentType(contentType);
			page.setText(new Utf8(""));
			new ParseUtil(conf).process(content.getUrl(), page);
			assertTrue(TableUtil.toString(page.getText()).startsWith(expectedMD5sum));
		}
	}
