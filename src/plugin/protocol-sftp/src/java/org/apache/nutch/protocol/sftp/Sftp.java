...
public class Sftp implements Protocol {
	...
	public ProtocolOutput getProtocolOutput(String url, WebPage page) {
		...
		try {
			...
			if (urlFile.endsWith(".htm") || urlFile.endsWith(".html")) {
				ProtocolOutput po = getFileProtocolOutput(sUrl, channelSftp, "text/html");
				return po;
			} else if (urlFile.endsWith(".pdf")) {
				ProtocolOutput po = getFileProtocolOutput(sUrl, channelSftp, "application/pdf");
				return po;
			} else {
	...
	private ChannelSftp getChannelSftp(URL url) throws InterruptedException {
		...
		try {
			...
			logger.error("Wait for getChannelSftp() interrupted for host: " + host, e);
			throw e;
		}
	}

	private void putChannelSftp(URL url, ChannelSftp cSftp) throws InterruptedException {
		String host = url.getHost();
		BlockingQueue<ChannelSftp> queue = channelSftpByHostMap.get(host);
		if (queue == null) {
			return;
		...
		try {
			...
			logger.error("Wait for putChannelSftp() interrupted for host: " + host, e);
			throw e;
		}
	}

	private ProtocolOutput getFileProtocolOutput(URL url, ChannelSftp channelSftp, String contentType) throws SftpException, IOException {
		byte[] bytes = null;
		...
		try {
			...
			logger.error("SftpException in getFileProtocolOutput(), file: " + url.getFile(), e);
			throw e;
		} catch (IOException e) {
			logger.error("IOException in getFileProtocolOutput(), file: " + url.getFile(), e);
			throw e;
		} finally {
			if (iStream != null) {
				iStream.close();
		...
		metadata.set(Response.LAST_MODIFIED, channelSftp.lstat(url.getFile()).getMtimeString());
		metadata.set(Response.LOCATION, urlStr);

		Content content = new Content(urlStr, urlStr, bytes, contentType, metadata, configuration);
		ProtocolOutput po = new ProtocolOutput(content);
		return po;
	}

	private ProtocolOutput getDirectoryProtocolOutput(URL url, ChannelSftp channelSftp) throws SftpException {
		try {
			...
			for (LsEntry entry : vector) {
				...
				if (!fileName.equals(".") && !fileName.equals("..")) {
					directoryList += "<a href=\"" + url + "/" + fileName + "\">" + count + "</a>\r\n";
				}
				count++;
			}
			...
			metadata.set(Response.CONTENT_LENGTH, String.valueOf(directoryList.length()));
			metadata.set(Response.LAST_MODIFIED, channelSftp.lstat(url.getFile()).getMtimeString());
			metadata.set(Response.LOCATION, url.toExternalForm());

			Content content = new Content(url.toExternalForm(), url.toExternalForm(), directoryList.getBytes(), "text/html", metadata,
					configuration);
			ProtocolOutput po = new ProtocolOutput(content);
			return po;
		} catch (SftpException e) {
	...
	public void setConf(Configuration arg0) {
		...
		synchronized (channelSftpByHostMap) {
			...
			try {
				...
				logger.error("Cannot create JSch session for user: " + user + ", host: " + server + ", port: " + port);
				return;
			}
			...
			try {
				...
				logger.error("Cannot connect to JSch session for user: " + user + ", host: " + server + ", port: " + port);
				return;
			}

			BlockingQueue<ChannelSftp> queue = new ArrayBlockingQueue<ChannelSftp>(1, true);
			try {
				queue.put(cSftp);
			} catch (InterruptedException e) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.nutch.protocol.Protocol#getRobotRules(java.lang.String, org.apache.nutch.storage.WebPage)
	 */
	@Override
