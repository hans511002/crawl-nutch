...
public class SegReParserJob extends NutchTool implements Tool {

	public static final Logger LOG = LoggerFactory.getLogger(SegReParserJob.class);
	...
	public static class SegmentParserAllReducer extends GoraReducer<Text, WebPage, String, WebPageSegment> {
		int rowCount = 0;
		int successCount = 0;
		int failCount = 0;
	...
	public static class SegmentParserAllMapper extends GoraMapper<String, WebPage, Text, WebPage> {
		SolrWriter solrWriter = null;
		private String batchID;
		private String batchTime;
		int rowCount = 0;
		...
		public void map(String key, WebPage page, Context context) throws IOException, InterruptedException {
			...
			LOG.info(key + " url:" + TableUtil.unreverseUrl(key));
			successCount++;
		}
	}

	public SegReParserJob() {

	}

	public SegReParserJob(Configuration conf) {
		setConf(conf);
	}
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		...
		if (getConf().getBoolean(SegParserJob.SEGMENT_INDEX_KEY, false)) {
			currentJob = new NutchJob(getConf(), "segment parse-indexAll:" + (this.getConf().get(NutchConstant.BATCH_ID_KEY)));
		} else {
			currentJob = new NutchJob(getConf(), "segment parseAll:" + (this.getConf().get(NutchConstant.BATCH_ID_KEY)));
		}
		...
		StorageUtils.initMapperJob(currentJob, fields, WebPage.class, Text.class, WebPage.class, SegmentParserAllMapper.class);
	...
	public int parse(boolean force, int numTasks) throws Exception {
	...
	public int run(String[] args) throws Exception {
		...
		int numTasks = 0;
		if (args.length < 1) {
			System.err.println("Usage: ParserJob [-crawlId <id>] <-batch batchId>"
					+ " <-startkey startKey> <-endkey endKey> [-numTasks N] [-toindex] [-index url]");
			return -1;
		}
		...
		return parse(force, numTasks);
	}

	public static void main(String[] args) throws Exception {
		final int res = ToolRunner.run(NutchConfiguration.create(), new SegReParserJob(), args);
		System.exit(res);
	}
}
