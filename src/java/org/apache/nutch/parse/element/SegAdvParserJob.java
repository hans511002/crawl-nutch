...
public class SegAdvParserJob extends NutchTool implements Tool {

	public static final Logger LOG = LoggerFactory.getLogger(SegAdvParserJob.class);
	...
	public static class SegParserMapper extends GoraMapper<String, WebPageIndex, Text, WebPageIndex> {
		...
		public void setup(Context context) throws IOException {
			...
			NutchConstant.setupSerialStepProcess(context.getConfiguration(), NutchConstant.BatchNode.segmentAdvParseNode, context, true);
		}
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			...
			NutchConstant.cleanupSerialStepProcess(context.getConfiguration(), NutchConstant.BatchNode.segmentAdvParseNode, context
					.getTaskAttemptID().getTaskID().toString(), true);
		}

		String getParseStatus(Context context) {
			return "read " + rowCount + " records";
		}
	...
	public SegAdvParserJob() {

	}

	public SegAdvParserJob(Configuration conf) {
		setConf(conf);
	}
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		...
		if (NutchConstant.preparStartJob(this.getConf(), NutchConstant.BatchNode.segmentParsNode, NutchConstant.BatchNode.segmentIndexNode,
				LOG, false) == 0)
			return null;
		LOG.info("SegmentParserJob: batchId: " + batchZKId);
		LOG.info("SegmentParserJob: forced reparse:\t" + getConf().getBoolean(FORCE_KEY, false));
		currentJob = new NutchJob(getConf(), "segmentParse:" + (this.getConf().get(NutchConstant.BATCH_ID_KEY)));
	...
	public static void main(String[] args) throws Exception {
		if (args.length == 0) {
			...
			query.setFields(StorageUtils.toStringArray(SegAdvParserJob.getFields(job)));
			...
			while (rs.next()) {
				...
				if (wps != null) {
					...
					if (doc != null) {
						SegAdvParserJob.LOG.info(key + " url:" + unreverseKey + " write to solr");
						solrWriter.write(doc);
					} else {
						SegAdvParserJob.LOG.warn(key + " url:" + unreverseKey + " doc is null");
					}
				}
				cout++;
			}
			...
			final int res = ToolRunner.run(NutchConfiguration.create(), new SegAdvParserJob(), args);
			System.exit(res);
		}
