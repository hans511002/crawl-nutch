...
public class IndexUtil {
	...
	public NutchDocument index(String key, WebPage page) {
		...
		Map<Utf8, ByteBuffer> meta = page.getMetadata();
		Utf8 keys = new Utf8(Metadata.KEYWORDS);
		Utf8 desc = new Utf8(Metadata.DESCRIPTION);
		if (meta.containsKey(keys))
			doc.add(Metadata.KEYWORDS, Bytes.toString(page.getFromMetadata(keys).array()));
		if (meta.containsKey(desc))
			doc.add(Metadata.DESCRIPTION, Bytes.toString(page.getFromMetadata(desc).array()));
