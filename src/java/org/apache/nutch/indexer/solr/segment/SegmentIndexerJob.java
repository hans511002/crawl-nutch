...
public class SegmentIndexerJob extends NutchTool implements Tool {
	...
	public static class SegIndexReducer extends GoraReducer<Text, WebPageSegmentIndex, String, WebPageSegment> {
		...
		private SolrWriter solrWriter = null;
	...
	public static class SegIndexMapper extends GoraMapper<String, WebPageSegmentIndex, Text, WebPageSegmentIndex> {
		...
		public void map(String key, WebPageSegmentIndex page, Context context) throws IOException, InterruptedException {
			...
			// }
			// if (doc != null) {
			// if ((rowCount - failCount) % commitSize == 0) {
			// System.out.println((rowCount - failCount) + " rows 解析用时" + (System.currentTimeMillis() - l) + "ms" + " 总用时："
			// + (System.currentTimeMillis() - all) + "ms");
			// l = System.currentTimeMillis();
			// }
			...
			// if ((rowCount - failCount) % commitSize == 0)
			// System.out.println((rowCount - failCount) + " rows 提交用时：" + (System.currentTimeMillis() - l) + "ms" + " 总用时："
			// + (System.currentTimeMillis() - all) + "ms");
			// } else {
			// failCount++;
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		...
		// currentJob = new NutchJob(getConf(), "segmentIndex:" + (this.getConf().get(NutchConstant.BATCH_ID_KEY)));
		// initMapperJob(currentJob, fields, WebPageSegmentIndex.class, String.class, NutchDocument.class, SegmentindexMapper.class, null,
		// false);
		// currentJob = SegmentIndexerJob.createIndexJob(getConf(), "segmentIndex:" + (this.getConf().get(NutchConstant.BATCH_ID_KEY)),
		// WebPageSegmentIndex.class, SegmentIndexMapper.class);
	...
	public int indexSolr(String solrUrl, boolean force, int numTasks) throws Exception {
		try {
			LOG.info("ParserJob: starting");
			run(ToolUtil.toArgMap(Nutch.ARG_SOLR, solrUrl, Nutch.ARG_FORCE, force, Nutch.ARG_NUMTASKS, numTasks));
	...
	public int run(String[] args) throws Exception {
		...
		return indexSolr(solrUrl, force, numTasks);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		// // query.setStartKey("com.qq.news:http/a/20130313");
		// // query.setEndKey("com.qq.news:http/a/20130314");
		...
		// if (doc != null) {
		// SegmentIndexerJob.LOG.info(key + " url:" + unreverseKey + " write to solr");
		// solrWriter.write(doc);
		// } else {
	...
	// public static <inV extends Persistent, map extends GoraMapper<String, inV, String, NutchDocument>> Job createIndexJob(
	// Configuration conf, String jobName, Class<inV> inValueClass, Class<map> indexerMapperClass) throws IOException,
	...
	//
	// Collection<WebPageSegment.Field> fields = getFields(job);
	// initMapperJob(job, fields, inValueClass, String.class, NutchDocument.class, indexerMapperClass, null, true);
