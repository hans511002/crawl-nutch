 ...
 * <li>Partition N among M map tasks. For example, if we have two map tasks the first map task will deal with solr documents from 0 - (N / 2
 * - 1) and the second will deal with documents from (N / 2) to (N - 1).</li>
 * </ol>
 * 
 * MapReduce:
 * <ul>
 * <li>Map: Identity map where keys are digests and values are {@link SolrRecord} instances(which contain id, boost and timestamp)</li>
 * <li>Reduce: After map, {@link SolrRecord}s with the same digest will be grouped together. Now, of these documents with the same digests,
 * delete all of them except the one with the highest score (boost field). If two (or more) documents have the same score, then the document
 * with the latest timestamp is kept. Again, every other is deleted from solr index.</li>
 * </ul>
 * 
 * Note that we assume that two documents in a solr index will never have the same URL. So this class only deals with documents with
 * <b>different</b> URLs but the same digest.
 */
public class SolrDeleteDuplicates extends Reducer<Text, SolrDeleteDuplicates.SolrRecord, Text, SolrDeleteDuplicates.SolrRecord> implements
		Tool {
	...
	public static class SolrInputFormat extends InputFormat<Text, SolrRecord> {
		...
		public List<InputSplit> getSplits(JobContext context) throws IOException, InterruptedException {
			...
			SolrServer solr = new HttpSolrServer(conf.get(SolrConstants.SERVER_URL));
		...
		@Override
		public RecordReader<Text, SolrRecord> createRecordReader(InputSplit split, TaskAttemptContext context) throws IOException,
			...
			solrQuery.setFields(SolrConstants.ID_FIELD, SolrConstants.BOOST_FIELD, SolrConstants.TIMESTAMP_FIELD,
	...
	public void setup(Context job) throws IOException {
		...
		solr = new HttpSolrServer(conf.get(SolrConstants.SERVER_URL));
	}
	...
	public void reduce(Text key, Iterable<SolrRecord> values, Context context) throws IOException {
		...
		while (iterator.hasNext()) {
			...
			if (solrRecord.getBoost() > recordToKeep.getBoost()
					|| (solrRecord.getBoost() == recordToKeep.getBoost() && solrRecord.getTstamp() > recordToKeep.getTstamp())) {
