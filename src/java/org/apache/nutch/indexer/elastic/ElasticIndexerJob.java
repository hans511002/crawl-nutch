...
/**
 * Indexer for elasticsearch. Uses bulk operations with flushing in the background, keeping track of elasticsearch responses by checking
 * after every flush. When a previous flush has not finished yet before the next bulk is full, it will wait for it. This mechanism will keep
 * the servers from overloading.
 */
public class ElasticIndexerJob extends IndexerJob {
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		...
		currentJob = createIndexJob(getConf(), "elastic-index [" + clusterName + "]", WebPage.class, IndexerAllMapper.class);
	...
	public void indexElastic(String clusterName, String batchId) throws Exception {
		run(ToolUtil.toArgMap(ElasticConstants.CLUSTER, clusterName, Nutch.ARG_BATCH, batchId));
	}
