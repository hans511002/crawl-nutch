...
import org.apache.nutch.indexer.solr.segment.SegmentIndexerJob;
...
public class Crawler extends NutchTool implements Tool {
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		...
		for (int i = 0; i < depth; i++) {
			...
			if (!this.getConf().getBoolean(NutchConstant.noSolrIndexNode, false)) {
				status.put(Nutch.STAT_PHASE, "index" + i);
				jobRes = runTool(SolrIndexerJob.class, args);
				if (jobRes != null) {
					subTools.put("index" + i, jobRes);
				}
			}
			status.put(Nutch.STAT_PHASE, "segParse" + i);
			jobRes = runTool(SegParserJob.class, args);
			if (jobRes != null) {
				subTools.put("segParse" + i, jobRes);
			}
			status.put(Nutch.STAT_PHASE, "segindex" + i);
			jobRes = runTool(SegmentIndexerJob.class, args);
			if (jobRes != null) {
				subTools.put("segindex" + i, jobRes);
			}
		}
		return results;
	}
	...
	public int run(String[] args) throws Exception {
		if (args.length == 0) {
			System.out
					.println("Usage: Crawler (<seedDir> | -continue) <-batch batchId> [-solr <solrURL>] [-segIndex <solrURL>] [-threads n] [-depth i] [-topN N] [-numTasks N] [-db] [-parse] ");
			return -1;
		}
		...
		for (int i = 0; i < args.length; i++) {
			if ("-threads".equals(args[i])) {
				...
				getConf().setBoolean(FetcherJob.PARSE_KEY, true);
			} else if ("-segIndex".equals(args[i])) {
				getConf().set(SolrConstants.SEGMENT_URL, args[++i]);
			} else if (args[i] != null) {
				seedDir = args[i];
			}
		}
