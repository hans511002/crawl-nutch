...
/**
 * Partition urls by host, domain name or IP depending on the value of the parameter 'partition.url.mode' which can be 'byHost', 'byDomain'
 * or 'byIP'
 */
public class URLPartitioner implements Configurable {
	...
	public static final String PARTITION_MODE_CFG = "byCFG";
	...
	public void setConf(Configuration conf) {
		...
		// }
		System.out.println("URLPartitioner mode=" + mode);
	...
	public int getPartition(String urlString, int numReduceTasks) {
		...
		if (url != null) {
			...
			} else if (mode.equals(PARTITION_MODE_IP)) { // MODE IP
				try {
	...
	public static class SelectorEntryPartitioner extends Partitioner<SelectorEntry, WebPage> implements Configurable {
		...
		public void setConf(Configuration conf) {
			this.conf = conf;
			if (conf.get(PARTITION_MODE_KEY, PARTITION_MODE_HOST).equals(PARTITION_MODE_CFG))
				cfgMode = true;
