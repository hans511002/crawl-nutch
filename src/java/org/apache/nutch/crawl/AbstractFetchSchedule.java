...
/**
 * This class provides common methods for implementations of {@link FetchSchedule}.
 *
...
public abstract class AbstractFetchSchedule extends Configured implements FetchSchedule {
	...
	/**
	 * Initialize fetch schedule related data. Implementations should at least set the <code>fetchTime</code> and
	 * <code>fetchInterval</code>. The default implementation sets the <code>fetchTime</code> to now, using the default
	...
	/**
	 * Sets the <code>fetchInterval</code> and <code>fetchTime</code> on a successfully fetched page. NOTE: this
	 * implementation resets the retry counter - extending classes should call super.setFetchSchedule() to preserve this
	 * behavior.
	 */
	@Override
	public void setFetchSchedule(String url, WebPage page, long prevFetchTime, long prevModifiedTime, long fetchTime,
			long modifiedTime, int state) {
		page.setRetriesSinceFetch(0);
	}

	/**
	 * This method specifies how to schedule refetching of pages marked as GONE. Default implementation increases
	 * fetchInterval by 50%, and if it exceeds the <code>maxInterval</code> it calls
	 * {@link #forceRefetch(Text, CrawlDatum, boolean)}.
	 * 
	 * @param url
	 *            URL of the page
	 * @param page
	 * @return adjusted page information, including all original information. NOTE: this may be a different instance
	 *         than
	 * @param datum
	 *            , but implementations should make sure that it contains at least all information from
	 * @param datum
	 *            .
	 */
	@Override
	...
	/**
	 * This method adjusts the fetch schedule if fetching needs to be re-tried due to transient errors. The default
	 * implementation sets the next fetch time 1 day in the future and increases the retry counter.
	 * 
	 * @param url
	 *            URL of the page
	 * @param page
	...
	/**
	 * This method provides information whether the page is suitable for selection in the current fetchlist. NOTE: a
	 * true return value does not guarantee that the page will be fetched, it just allows it to be included in the
	 * further selection process based on scores. The default implementation checks <code>fetchTime</code>, if it is
	 * higher than the
	 * 
	 * @param curTime
	 *            it returns false, and true otherwise. It will also check that fetchTime is not too remote (more than
	 *            <code>maxInterval</code), in which case it lowers the interval and returns true.
	 * @param url
	 *            URL of the page
	 * @param page
	 * @param curTime
	 *            reference time (usually set to the time when the fetchlist generation process was started).
	 * @return true, if the page should be considered for inclusion in the current fetchlist, otherwise false.
	 */
	@Override
	...
	/**
	 * This method resets fetchTime, fetchInterval, modifiedTime, retriesSinceFetch and page signature, so that it
	 * forces refetching.
	 * 
	 * @param url
	 *            URL of the page
	 * @param page
	 * @param asap
	 *            if true, force refetch as soon as possible - this sets the fetchTime to now. If false, force refetch
	 *            whenever the next fetch time is set.
	 */
	@Override
