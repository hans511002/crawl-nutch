...
public class FetcherReducer extends GoraReducer<IntWritable, FetchEntry, String, WebPageIndex> {

	public static final Logger LOG = FetcherJob.LOG;

	private final AtomicInteger activeThreads = new AtomicInteger(0);
	private final AtomicInteger spinWaiting = new AtomicInteger(0);
	private static final String fetcherServerDelayKey = "fetcher.server.delay";
	...
	private boolean storingHeaders;
	...
	private static class FetchItem {
		...
		public FetchItem(String url, WebPageIndex page, URL u, String queueID) {
			this.page = page;
			this.url = url;
			this.u = u;
		...
		/**
		 * Create an item. Queue id will be created based on <code>queueMode</code> argument, either as a protocol + hostname pair, protocol
		 * + IP address pair or protocol+domain pair.
		 */
		public static FetchItem create(String url, WebPageIndex page, String queueMode) {
			String queueID;
			URL u = null;
			try {
				u = new URL(url);
	...
	/**
	 * This class handles FetchItems which come from the same host ID (be it a proto/hostname or proto/IP pair). It also keeps track of
	 * requests in progress and elapsed time between requests.
	 */
	private static class FetchItemQueue {
		...
		public void finishFetchItem(FetchItem it, boolean asap) {
			finishFetchItem(it, asap, false);
		}

		public void finishFetchItem(FetchItem it, boolean asap, boolean asapUrlCfg) {
			if (it != null) {
				if (asapUrlCfg) {
					long val = (long) (it.page.nodeConfig.getFloat(fetcherServerDelayKey) * 1000);
					inProgress.remove(it);
					setEndTime(System.currentTimeMillis(), val, asap);
				} else {
					inProgress.remove(it);
		...
		private void setEndTime(long endTime, long intval, boolean asap) {
			if (!asap)
				nextFetchTime.set(endTime + (maxThreads > 1 ? minCrawlDelay : crawlDelay) + intval);
			else
				nextFetchTime.set(endTime + intval);
		}
	...
	/**
	 * Convenience class - a collection of queues that keeps track of the total number of items, and provides items eligible for fetching
	 * from any queue.
	 */
	private static class FetchItemQueues {
		...
		public FetchItemQueues(Configuration conf) throws IOException {
			...
			this.crawlDelay = (long) (conf.getFloat(fetcherServerDelayKey, 1.0f) * 1000);
		...
		public void addFetchItem(String url, WebPageIndex page) {
		...
		public void finishFetchItem(FetchItem it, boolean asap, boolean asapUrlCfg) {
			...
			fiq.finishFetchItem(it, asap, asapUrlCfg);
		}
			...
			if (fiq == null) {
				...
				if (useHostSettings) {
					...
					try {
						...
						if (host != null) {
							fiq = new FetchItemQueue(conf, host.getInt("q_mt", maxThreads), host.getLong("q_cd", crawlDelay), host.getLong(
									"q_mcd", minCrawlDelay));
						}
					} catch (IOException e) {
	...
	private class FetcherThread extends Thread {
			...
			try {
				...
				while (true) {
					...
					try {
						...
						UrlPathMatch urlSegment = NutchConstant.urlcfg.match(fit.url);// 获取配置项
						if (urlSegment == null) {
							throw new Exception("FetcherReducer 495: url:" + fit.url + " is not in urlConfig");
						}
						UrlNodeConfig nodeValue = (UrlNodeConfig) urlSegment.getNodeValue();
						fit.page.nodeConfig = nodeValue;
						// fetch the page
						...
						// if (!rules.isAllowed(fit.u)) { // 不允许抓取
						// // unblock
						...
						// unblock queue
						fetchQueues.finishFetchItem(fit, false, true);// 采用主机异步 + URL配置间隔
						fit.page.nodeConfig = null;
						...
						case ProtocolStatusCodes.ROBOTS_DENIED:
							LOG.warn("FetcherReducer 572: set URL:" + fit.url + " fetchinterval from " + fit.page.getFetchInterval()
									+ " to " + (fit.page.getFetchInterval() * 5 + 30 * FetchSchedule.SECONDS_PER_DAY));
							fit.page.setFetchInterval(fit.page.getFetchInterval() * 5 + 30 * FetchSchedule.SECONDS_PER_DAY);
					...
					} catch (final Exception t) { // unexpected exception
						System.out.print("FetcherReducer 588:");
						t.printStackTrace(System.out);
						// unblock
						fetchQueues.finishFetchItem(fit);
						logFetchFailure(fit.url, fit.url + " error:" + t.getMessage());
						LOG.error("Unexpected error for " + fit.url, t);
						output(fit, null, ProtocolStatusUtils.STATUS_FAILED, CrawlStatus.STATUS_RETRY);
					}
				}
			} catch (final Exception e) {
				System.out.print("FetcherReducer 599:");
		...
		private void handleRedirect(String url, String newUrl, boolean temp, String redirType, WebPage page) throws URLFilterException,
		...
		private void output(FetchItem fit, Content content, ProtocolStatus pstatus, byte status) throws IOException, InterruptedException {
			...
			fit.page.setPrevFetchTime(curTime);
			fit.page.setFetchTime(curTime);
			if (pstatus != null) {
			...
			if (storingHeaders && content != null) {
				Metadata data = content.getMetadata();
				String[] names = data.names();
				for (String name : names) {
					fit.page.putToHeaders(new Utf8(name), new Utf8(data.get(name)));
				}
			}
			...
			key = NutchConstant.getWebPageIndexUrl(batchID, key);// indexKey + key;
			fit.page.setStatus(status);
			System.out.println(" status:" + status + "  len:"
					+ (content != null && content.getContent() != null ? content.getContent().length : "null") + " url:"
					+ (content != null && content.getContent() != null ? content.getBaseUrl() : "") + " key:" + key);
	...
	private void reportAndLogStatus(Context context, float actualPages, int actualBytes, int totalSize) throws IOException {
		...
		status.append(Math.round((((float) bytes.get())) / 1024) / elapsed).append(" ");
		status.append(Math.round(((float) actualBytes)) / 1024).append(" kb/s, ");
	...
	public void run(Context context) throws IOException, InterruptedException {
		...
		storingHeaders = conf.getBoolean("fetcher.store.headers", true);
		if (parse) {
		...
		do { // wait for threads to exit
			...
			if (throughputThresholdTimeLimit < System.currentTimeMillis() && throughputThresholdPages != -1) {
				...
				if (pagesLastSec < throughputThresholdPages) {
					...
					LOG.warn(Integer.toString(throughputThresholdCurrentSequence) + ": dropping below configured threshold of "
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		FetcherJob.LOG.info("fetcherReducer-批次ID：" + batchID + "  进入时间:" + (new Date().toLocaleString()) + " 批次时间：" + batchTime);
		NutchConstant.setupSerialStepProcess(context.getConfiguration(), NutchConstant.BatchNode.fetchNode, context, false);
	}

	@Override
	protected void cleanup(Context context) throws IOException, InterruptedException {
		FetcherJob.LOG.info("fetcherReducer-批次ID：" + batchID + "  退出时间:" + (new Date().toLocaleString()) + " 批次时间：" + batchTime);
		NutchConstant.cleanupSerialStepProcess(context.getConfiguration(), NutchConstant.BatchNode.fetchNode, context.getTaskAttemptID()
				.getTaskID().toString(), false, true, pages.longValue() - errors.longValue());
	}
}
