...
public class DbReader {
	...
	public Iterator<Map<String, Object>> iterator(String[] fields, String startKey, String endKey, String batchId) throws Exception {
	...
	private class DbIterator implements Iterator<Map<String, Object>> {
		...
		DbIterator(Result<String, WebPage> res, String[] fields, String batchId) throws Exception {
			this.res = res;
			if (batchId != null) {
				...
				do {
					...
					if (NutchJob.shouldProcess(mark, batchId)) {
						...
						if (LOG.isDebugEnabled()) {
							LOG.debug("Skipping " + TableUtil.unreverseUrl(res.getKey()) + "; different batch id");
						}
						hasNext = res.next();
					}
				} while (hasNext);
				...
				if ("metadata".equals(f)) {
					...
					if (metadata != null) {
						...
						while (iterator.hasNext()) {
							...
							simpleMeta.put(entry.getKey().toString(), Bytes.toStringBinary(entry.getValue().array()));
						}
					}
