...
public class SegmentExporterJob extends NutchTool implements Tool {

	public static final Logger LOG = LoggerFactory.getLogger(SegmentExporterJob.class);
	...
	public static class SegExportMapper extends GoraMapper<String, WebPageSegmentIndex, Text, WebPageSegmentIndex> {
	...
	public SegmentExporterJob() {

	}

	public SegmentExporterJob(Configuration conf) {
		setConf(conf);
	}
	...
	public Map<String, Object> run(Map<String, Object> args) throws Exception {
		...
		currentJob = new NutchJob(getConf(), "[" + (this.getConf().get(NutchConstant.BATCH_ID_KEY)) + "]segmentExport[" + gids + "]");
		currentJob.getConfiguration().set(NutchConstant.STEPZKBATCHTIME, new Date().toLocaleString());
		Collection<WebPageSegment.Field> fields = SegmentExporterJob.getFields(currentJob);
		...
		WebPageSegment.initMapperJob(currentJob, fields, WebPageSegmentIndex.class, Text.class, WebPageSegmentIndex.class,
				SegExportMapper.class, null, true);
	...
	public int run(String[] args) throws Exception {
		boolean force = false;
		if (args.length < 1) {
			System.err.println("Usage: SegmentExporterJob <-batch batchId> [-crawlId <id>] [-force]");
			return -1;
		}
		int numTasks = 0;
	...
	public static void main(String[] args) throws Exception {
		if (args.length == 0) {
			...
			final int res = ToolRunner.run(NutchConfiguration.create(), new SegmentExporterJob(), args);
			System.exit(res);
		}
	}
