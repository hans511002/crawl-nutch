 ...
 * 
 * <p>
 * A caller will often have some extra information about what the encoding might be (e.g. from the HTTP header or HTML meta-tags, often
 * wrong but still potentially useful clues). The types of clues may differ from caller to caller. Thus a typical calling sequence is:
 * <ul>
...
public class EncodingDetector {
	...
	static {
		...
		/*
		 * the following map is not an alias mapping table, but maps character encodings which are often used in mislabelled documents to
		 * their correct encodings. For instance, there are a lot of documents labelled 'ISO-8859-1' which contain characters not covered by
		 * ISO-8859-1 but covered by windows-1252. Because windows-1252 is a superset of ISO-8859-1 (sharing code points for the common
		 * part), it's better to treat ISO-8859-1 as synonymous with windows-1252 than to reject, as invalid, documents labelled as
		 * ISO-8859-1 that have characters outside ISO-8859-1.
		 */
	...
	public void autoDetectClues(WebPage page, boolean filter) {
		autoDetectClues(page.getContent(), page.getContentType(), parseCharacterEncoding(page.getFromHeaders(CONTENT_TYPE_UTF8)), filter);
	}

	public void autoDetectClues(ByteBuffer dataBuffer, Utf8 typeUtf8, String encoding, boolean filter) {
		byte[] data = dataBuffer.array();
		String type = TableUtil.toString(typeUtf8);

		if (minConfidence >= 0 && DETECTABLES.contains(type) && data.length > MIN_LENGTH) {
	 ...
	 *            URL's row
	 * @param defaultValue
	 *            Default encoding to return if no encoding can be detected with enough confidence. Note that this will <b>not</b> be
	...
	public String guessEncoding(String baseUrl, String defaultValue) {
		/*
		 * This algorithm could be replaced by something more sophisticated; ideally we would gather a bunch of data on where various clues
		 * (autodetect, HTTP headers, HTML meta tags, etc.) disagree, tag each with the correct answer, and use machine learning/some
		 * statistical method to generate a better heuristic.
		 */
		...
		/*
		 * Go down the list of encoding "clues". Use a clue if: 1. Has a confidence value which meets our confidence threshold, OR 2.
		 * Doesn't meet the threshold, but is the best try, since nothing else is available.
		 */
	...
	/*
	 * Strictly for analysis, look for "disagreements." The top guess from each source is examined; if these meet the threshold and
	 * disagree, then we log the information -- useful for testing or generating training data for a better heuristic.
	 */
	...
	/**
	 * Parse the character encoding from the specified content type header. If the content type is null, or there is no explicit character
	 * encoding, <code>null</code> is returned. <br />
	 * This method was copied from org.apache.catalina.util.RequestUtil, which is licensed under the Apache License, Version 2.0 (the
	 * "License").
	 * 
	 * @param contentType
	 ...
	 * System.err.println("Usage: EncodingDetector <file>"); System.exit(1); }
	 * 
	 * Configuration conf = NutchConfiguration.create(); EncodingDetector detector = new EncodingDetector(NutchConfiguration.create());
	 * 
	 * // do everything as bytes; don't want any conversion BufferedInputStream istr = new BufferedInputStream(new
	 * FileInputStream(args[0])); ByteArrayOutputStream ostr = new ByteArrayOutputStream(); byte[] bytes = new byte[1000]; boolean more =
	 * true; while (more) { int len = istr.read(bytes); if (len < bytes.length) { more = false; if (len > 0) { ostr.write(bytes, 0, len); }
	 * } else { ostr.write(bytes); } }
	 * 
	 * byte[] data = ostr.toByteArray(); MimeUtil mimeTypes = new MimeUtil(conf);
	 * 
	 * // make a fake Content Content content = new Content("", "", data, "text/html", new Metadata(), mimeTypes);
	 * 
	 * detector.autoDetectClues(content, true); String encoding = detector.guessEncoding(content,
	 * conf.get("parser.character.encoding.default")); System.out.println("Guessed encoding: " + encoding); }
	 */
